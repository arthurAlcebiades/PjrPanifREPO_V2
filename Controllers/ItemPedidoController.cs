...
namespace PrjPaniMVCv2.Controllers
{
    public class ItemPedidoController : Controller
    {
        ...
        public ItemPedidoController(PaniContext context)
        {
            ...
            if (ped.HasValue)
            {
                if (_context.Pedidos.Any(p => p.IdPedido == ped))
                {
                    var pedido = await _context.Pedidos
                        .Include(p => p.Cliente)
                        .Include(p => p.ItensPedido.OrderBy(i => i.Produto.NomeProduto))
            ...
            if (ped.HasValue)
            {
                if (_context.Pedidos.Any(p => p.IdPedido == ped))
                {
                    var produtos = _context.Produtos
                        ...
                        .Select(p => new { p.IdProduto, p.TipoUnidade, NomePreco = $"{p.NomeProduto} ({p.TipoUnidade} {p.Preco:C})" })
                        .AsNoTracking().ToList();
        ...
        public async Task<IActionResult> Cadastrar([FromForm] ItemPedidoModel itemPedido)
        {
            ...
            else
            {
                var produtos = _context.Produtos
                        .OrderBy(x => x.NomeProduto)
                        .Select(p => new { p.IdProduto, NomePreco = $"{p.NomeProduto} ({p.Preco:C})" })
                        .AsNoTracking().ToList();
                var produtosSelectList = new SelectList(produtos, "IdProduto", "NomePreco");
                ViewBag.Produtos = produtosSelectList;
